...
from torch_geometric.nn.pool.select import topk,SelectTopK
...
class MVPool(torch.nn.Module):
    def __init__(self, in_channels, ratio, args, negative_slop=0.2):
        ...
        # self.topk = TopKPooling(in_channels, ratio)
        # self.topk = SelectTopK(in_channels, ratio)
        self.select_topk = SelectTopK(self.nhid, ratio=ratio)
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        # Graph Pooling
        original_x = x
        # print(batch,score,self.ratio)
        # 之前： topk(score, batch)
        # perm = topk(score, batch)
        #TODO: 瞎写
        # perm = topk(score, self.ratio, batch)
        # 使用 SelectTopK 实例
        select_output = self.select_topk(score, batch)
