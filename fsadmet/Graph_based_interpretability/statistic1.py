...
sys.path.append('/root/codes/MolFeSCue-master-2/fsadmet')

from model import *
# from utils import *
from edgeshape1 import *
import pandas as pd
import pickle
...
from model.models.model import Graph
if not os.path.exists('/root/codes/MolFeSCue-master-2/tox-10/img_1'):
    os.makedirs('/root/codes/MolFeSCue-master-2/tox-10/img_1')
...
# Read CSV file
df = pd.read_csv('/root/codes/Graph-main/tox21/'+'toxic-10.csv')
...
def predict_and_calculate_shapley(model, device, loader, target_class=0):
    ...
    with torch.no_grad():
        for idx, data in enumerate(loader):
            ...
            img_path = os.path.join('/root/codes/Graph-main/tox-10/img_1', f'{safe_smiles}.png')
            print(img_path)
            img.save(img_path)
...
# smiles_list = df['SR-HSE'].tolist()

# Preparing for prediction
processed_test = '/root/codes/Graph-main/tox21/tox-test.pth'
if not os.path.isfile(processed_test):
    ...
    batchestest1 = list()
    from structuralremap_construction.data.featurize import collate_with_circle_index2
    for batch_idx, data in enumerate(batchestest):
        data = collate_with_circle_index2(data)
        data.edge_attr = None
    ...
    model = Graph(args=args,num_features_xd=128,dropout=0.2,aug_ratio=0.4,weights=[0.4, 0.4, 0.2, 0.6, 0.3, 0.1]).to(args.device)
    model_file_name = '/root/codes/MolFeSCue-master-2/model_output/model_epoch_1500.pth'
    ...
    print(f'total_shapley_values',total_shapley_values)
    print(f'high_shapley_bond_types',high_shapley_bond_types)
    print(f'high_shapley_bond_types',high_shapley_bond_neighbour_info)
    ...
    # Save data to pickle
    with open('/home/dell/mxq/toxic_mol/model/MS2/Graph_based_interpretability/data/rat/bond_data_1.pkl', 'wb') as f:
